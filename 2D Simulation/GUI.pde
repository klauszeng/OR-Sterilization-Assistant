...
void mouseDragged() // executed when the mouse is dragged (while mouse buttom pressed)
  ...
  if (keyPressed) {
      ...
      if (key=='t'); surgeon.moveWithMouse(); //print(surgeon.x, surgeon.y);//P5.dragAll(); // move all vertices
...
String title ="Team UHE Demo",            name ="Student: Jianming && Luka && Gabriel ",
       subtitle = "Base code for operating room",
       
       menu="save for later",
       guide="save for later"; // help info
